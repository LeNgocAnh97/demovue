    new Vue({
        ...
        methods : {
            addCart : function(product){
                var checkAddCart = this.checkAddCart(product);
                    if(checkAddCart == null){
                        this.cart.item.push({
                        product : product,
                        quality : 1
                        });
            ...
            // Check trùng
            checkAddCart : function(product){
                for(var i = 0 ; i < this.cart.item.length ; i++){
                      if( this.cart.item[i].product.id ==  product.id )  
                            return this.cart.item[i];
                }
                return null;
            },

            congQualityItem : function(product){
                  product.quality++;      
            },
            truQualityItem : function(product){
                product.quality--;
            },
            deleteItem : function(product, index){
